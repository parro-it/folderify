...
function folderify(file) {
  ...
  function buildOriginalSource(folder, sourceFilter, options, stream) {
    ...
    fs.readdirSync(folder)
      .filter(_sourceFilter.test.bind(_sourceFilter))
      .map(fileName => {
        stream.emit('file', path.join(folder, fileName));
      });
  ...
  function parse(stream) {
    ...
    const output = falafel(data, {ecmaVersion: 6}, function(node) {
      ...
      if (node.type === 'CallExpression' && node.callee && ifNames[node.callee.name] === true) {
        ...
        const originalSource = buildOriginalSource(folder, filesFilter, options, stream);
